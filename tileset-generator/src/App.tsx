...
function App() {
  ...
  useEffect(() => {
    ...
    loadImage('/blob-mask.webp').then(image => {
      setConfig(prev => ({
        ...prev,
        maskImage: image
      }));
    ...
    loadImage('/snow-texture.webp').then(image => {
      setConfig(prev => ({
        ...prev,
        textureImage: image
      }));
  ...
  const handleImageUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>, type: 'mask' | 'texture') => {
  ...
  const downloadBlobTileset = useCallback(async (index: number) => {
    const canvas = canvasRefs.current[index];
    if (!canvas) return;
    ...
    link.download = `${namePrefix}${index + 1}.webp`; // Add 1 to index for 1-based numbering
    ...
    <div className="w-full h-full flex flex-col items-center p-5 gap-5 min-h-screen bg-gray-900 text-gray-100">
      ...
      <div className="flex gap-5 mb-5">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            ...
            />
            <span className="text-xs text-gray-400">Default: blob-mask.webp</span>
          </div>
        </div>
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1">
            <input
              ...
              className="file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-gray-700 file:text-gray-100 hover:file:bg-gray-600 file:cursor-pointer text-gray-300"
            />
            <span className="text-xs text-gray-400">Default: snow-texture.webp</span>
          </div>
        </div>
        <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-5 mb-5">
        {config.maskImage && (
          <div>
            ...
            <img src={config.maskImage.src} alt="Mask" className="max-w-[200px] max-h-[200px] object-contain bg-gray-800 rounded-lg p-2" />
          </div>
        )}
        {config.textureImage && (
          <div>
            <h3 className="text-xl mb-2 text-gray-200">Texture Preview</h3>
            <img src={config.textureImage.src} alt="Texture" className="max-w-[200px] max-h-[200px] object-contain bg-gray-800 rounded-lg p-2" />
          </div>
        )}
      </div>
      ...
      {blobTilesets.length > 0 && (
        <div className="grid grid-cols-3 gap-4 p-5 border border-gray-700 rounded-lg bg-gray-800">
          <h2 className="text-2xl mb-3 text-gray-100 col-span-3">Generated Blob Tilesets</h2>
          {blobTilesets.map((blobTileset, index) => (
            <div key={`${blobTileset.sourceX}-${blobTileset.sourceY}`} className="flex flex-col items-center gap-2">
              ...
              />
              <button 
                onClick={() => downloadBlobTileset(index)}
              ...
              >
                Download {namePrefix}{index + 1}.webp
              </button>
            </div>
          ))}
        </div>
